...
# Основная команда создания бэкапа и стягивания с плавающего 192.168.87.3
/usr/bin/rdiff-backup --remote-schema 'ssh -C %s rdiff-backup --server' -v5 root@192.168.87.3::/stg/8tb/share/ /backups/jira-data/

# Очистка старых бэкапов (оставляем последние 12)
echo "Удаление старых бекапов, оставляем последние 12-ть: " /usr/bin/rdiff-backup --force --remove-older-than 12B /backups/jira-data/
