...
<br/> Проверяй версии по ссылке https://grafana.com/grafana/download или https://github.com/grafana/grafana/releases
...
Первый вход через UI с данными  **login: `admin` / Pass: `admin`**
...
## Права доступа на файлы с логами
Promtail работает под пользователем `promtail`, который состоит в группе `adm`, но файлы логов принадлежали `root:root` с правами 644.

Promtail должен иметь права на файлы с логами:
- /var/log/auth.log
- /var/log/syslog
- /var/log/grafana/*.log
- и др. подключаемые
...
```bash
# Дать права на чтение группе adm
sudo chmod g+r /var/log/auth.log /var/log/syslog
sudo chmod g+rx /var/log/grafana
sudo chmod g+r /var/log/grafana/*.log

# Или изменить владельца на promtail
sudo chown promtail:adm /var/log/auth.log /var/log/syslog
sudo chown -R promtail:adm /var/log/grafana/
```

Проверка
```bash
# Проверить что promtail теперь видит файлы
curl -s http://localhost:9080/targets | grep -A2 "job_header"
...
logcli labels --addr=http://192.168.195.238:3100 job  #на машине с собираемыми логами
...
### 1.2. Labels - Работа с метками
Проверка метрик на примере того, когда машины в разных сетях.

Машина с Grafana 192.168.87.179 (zerotier 192.168.195.238)
```bash
root@runelfrontdev /var/log
16:24:07 # echo 'TEST LOG after Loki ready - $(date)' | sudo tee -a /var/log/auth.log
TEST LOG after Loki ready - $(date)

root@runelfrontdev /var/log
16:24:49 # curl -s http://localhost:9080/metrics | grep -E 'promtail_sent_(entries|bytes)_total'
# HELP promtail_sent_bytes_total Number of bytes sent.
# TYPE promtail_sent_bytes_total counter
promtail_sent_bytes_total{host="localhost:3100"} 1785
# HELP promtail_sent_entries_total Number of log entries sent to the ingester.
# TYPE promtail_sent_entries_total counter
promtail_sent_entries_total{host="localhost:3100"} 8
```

Машина с собираемыми логами 91.207.75.89 (zerotier 192.168.195.122)
```bash
root@starodubtcevs:~/programs# curl -v http://192.168.195.238:3100/ready
*   Trying 192.168.195.238:3100...
* Connected to 192.168.195.238 (192.168.195.238) port 3100
> GET /ready HTTP/1.1
> Host: 192.168.195.238:3100
...
< Date: Mon, 15 Dec 2025 13:18:51 GMT
< Content-Length: 6
< 
ready
* Connection #0 to host 192.168.195.238 left intact

root@starodubtcevs:~/programs# curl -s http://localhost:9080/metrics | grep -E 'promtail_sent_(entries|bytes)_total'
# HELP promtail_sent_bytes_total Number of bytes sent.
# TYPE promtail_sent_bytes_total counter
promtail_sent_bytes_total{host="192.168.195.238:3100"} 0
# HELP promtail_sent_entries_total Number of log entries sent to the ingester.
# TYPE promtail_sent_entries_total counter
promtail_sent_entries_total{host="192.168.195.238:3100"} 0

root@starodubtcevs:~/programs# echo 'TEST LOG after Loki ready - $(date)' | sudo tee -a /var/log/auth.log
TEST LOG after Loki ready - $(date)

root@starodubtcevs:~/programs# curl -s http://localhost:9080/metrics | grep -E 'promtail_sent_(entries|bytes)_total'
# HELP promtail_sent_bytes_total Number of bytes sent.
# TYPE promtail_sent_bytes_total counter
promtail_sent_bytes_total{host="192.168.195.238:3100"} 110
# HELP promtail_sent_entries_total Number of log entries sent to the ingester.
# TYPE promtail_sent_entries_total counter
promtail_sent_entries_total{host="192.168.195.238:3100"} 1
```
...
server:
  ...
  http_listen_address: 0.0.0.0  # слушать все адреса, если у нас машина в другой сети
...
common:
  ...
  ring:
    ...
    #instance_addr: 192.168.195.238  #Если машина в другой сети
    kvstore:
      store: inmemory
...
server:
  #http_listen_address: 127.0.0.1  # отключение web интерфейса
  http_listen_address: 0.0.0.0  # или 192.168.195.209
  http_listen_port: 9080
  grpc_listen_port: 0
...
**Если вы уже зашли в Grafana и поменяли пароль для админа, настроили все приложения сервера, то можно уже создавать дашборды.**

## Коннект с LOKI плагином
- Выбираем в левой панели **`Connections`** -> **`Data Sources`**;
- В окне `loki`, зплняем поле `Connection`, указывая в формает **`http://192.168.87.179:3100`**;
- Внизу страницы подвтерждаем сохрание, должны увидеть **`Data source successfully connected`**.

## Создание Dashboards
- Переходи на вкладку `Dashboards` в левой панели;
- создаём новый Dashboard;
- заполняем Label filters, перечисляя через символ `+`;
- нажимаем кнопку `Add query` для добавления очередей в одном дашборде;
- или воспользоваться кнопкой `Label browser`.
Т.о. будет создан дашборд, в который логи будут добавлять не совсем сразу.

## Drilldown -> Logs
Переходим на вкладку `Drilldown -> Logs`, и там мы должны увидеть все добавленные логи по-хорошему.


<br/> 


# Восстановление пароля Grafana

### Переходим и смотрим содержимое
```bash
root@loki-grafana ~ > cd /etc/grafana/
root@loki-grafana /etc/grafana > ll
total 107k
drwxr-xr-x   3 root root    4.1k Sep 23 14:49 ./
drwxr-xr-x 119 root root    4.1k Oct  1 06:36 ../
drwxr-xr-x   7 root grafana 4.1k Sep 22 13:09 provisioning/
-rw-r-----   1 root grafana  88k Sep 23 14:49 grafana.ini
-rw-r-----   1 root grafana 3.2k Sep 22 13:09 ldap.toml
```


### Способ 1: Сброс пароля через базу данных (рекомендуемый)

Сначала найдём базу данных Grafana:
```bash
sudo find / -name "grafana.db" 2>/dev/null
```

Обычно она находится в одной из этих папок:
- `/var/lib/grafana/grafana.db`
- `/etc/grafana/grafana.db`

Если найдёте файл базы данных, используйте SQLite для сброса пароля:

```bash
# Предположим, база в /var/lib/grafana/grafana.db
sudo sqlite3 /var/lib/grafana/grafana.db
```

В интерактивной консоли SQLite выполните:
```sql
UPDATE user SET password = '59acf18b94d7eb0694c61e60ce44c110c7a683ac6a8f09580d626f90f4a242000746579358d77dd9e570e83fa24faa88a8a6', salt = 'F3FAxVm33R' WHERE login = 'admin';
.exit
```

После этого пароль для `admin` будет `admin`.

### Способ 2: Через конфигурационный файл

Отредактируем `grafana.ini`:
```bash
sudo mcedit /etc/grafana/grafana.ini
```

Найдите раздел `[security]` и добавьте/измените строки:
```ini
[security]
admin_user = admin
...
Сохраните файл (F2, Y, Enter, F10) и перезапустите Grafana:
```bash
...
или одной командой:
```bash
echo -e "\n[security]\nadmin_user = admin\nadmin_password = admin" | sudo tee -a /etc/grafana/grafana.ini
```

### Способ 3: Быстрая проверка текущих настроек

Сначала посмотрим, работает ли Grafana:
```bash
sudo systemctl status grafana-server
```

Проверим, есть ли уже настройки администратора в конфиге:
```bash
sudo grep -A 2 -B 2 "admin_password" /etc/grafana/grafana.ini
```

### После восстановления доступа:

1. **Зайдите на http://192.168.87.209:3000** с логином `admin` и паролем `admin`
2. **Сразу поменяйте пароль** на новый
3. **Проверьте настройки сессии** чтобы избежать повторения проблемы
<br/>



# Создание пользователя в Grafana

## Просмотр пользователей

1. **В левом меню** нажмите на **шестерёнку (Configuration)**
2. **Выберите "Users"** 
3. **Вы увидите список всех пользователей** с информацией:
   - Login (логин)
   - Email
   - Name
   - Organization
...
1. **В разделе Users** нажмите **"New user"**
2. **Заполните данные:**
   - Name (имя)
   - Email
   - Login (логин для входа)
   - Password
   - Organization (обычно "Main Org.")
3. **Назначьте роль:**
   - **Viewer** - только просмотр
   - **Editor** - просмотр и редактирование дашбордов
   - **Admin** - полный доступ

### Способ 2: Настройки аутентификации
В **Configuration → Authentication** можно настроить:
- **Авторизацию через LDAP/Active Directory**
...
- **Editor** - может создавать/редактировать дашборды
- **Admin** - полный доступ ко всем настройкам

### Дополнительные настройки:
1. **Organization settings** - управление организациями
2. **Preferences** - настройки организации по умолчанию
3. **Authentication** - настройки входа

## Полезные команды через API

Если нужно автоматизировать:
```bash
# Создание пользователя через API
curl -X POST http://admin:ваш_пароль@192.168.87.209:3000/api/admin/users \
  -H "Content-Type: application/json" \
  -d '{"name":"User", "email":"user@example.com", "login":"user", "password":"password"}'
```
...
1. **Создайте отдельного пользователя** для повседневной работы (не используйте admin)
2. **Назначайте минимально необходимые права**
3. **Для командной работы** настройте LDAP или OAuth


<p align="center">
  <img src="https://github.com/sherbettt/BASH-cheats/blob/main/images/Grafana_Example.png" alt="Grafana Example">
</p>


<!-- [![Grafana Example](https://github.com/sherbettt/BASH-cheats/blob/main/images/Grafana_Example.png)](https://github.com/sherbettt/BASH-cheats/blob/main/images/Grafana_Example.png) -->

-----------------------
<br>


## Grafana через Teleport
Данная опция относится к самому Teleport, но для общего понимания знаний, оставлю и здесь.

### 1.0 Настройки в Teleport. Подготовка
```bash
#Переходим в /etc/teleport.d
cd /etc/teleport.d

# создаём конфигурацию grafana-app.yaml
mcedit grafana-app.yaml
```

### 1.1 Настройки в Teleport. Конфиги

<details>
<summary>❗конфиги в Teleport❗</summary>

**Содержание teleport.yaml**
```yaml
version: v3
teleport:
...
```
Закоментированая часть с grafana в моём случае не сработало, почему потребовалось создавать отдельный файл.

**Содержание grafana-app.yaml**
```yaml
kind: app
version: v3
metadata:
...
### 2.0 Настройка конфигов на Grafana сервере
***Редактируем секции /etc/grafana/grafana.ini**
```bash
mcedit /etc/grafana/grafana.ini
```
```ini
[server]
# Этот адрес должен совпадать с public_addr, который указан в конфиге приложения Teleport
root_url = https://grafana.teleport.runtel.org

[auth]
# Отключаем проверку Referer, чтобы Grafana принимала запросы от Teleport
disable_login_form = false
```
```bash
# проверка
sed -n '37,39p' /etc/grafana/grafana.ini
sed -n '37,39p;609,611p' grafana.ini
```

***Опционально!*** добавить в конце файла **grafana.ini**
```ini
[auth.generic_oauth]
enabled = true
name = Runtel SSO
...
auto_login = false
client_id = grafana
client_secret = <скопируй из Keycloak>
auth_url = https://sso.runtel.ru:8443/realms/runtel/protocol/openid-connect/auth
token_url = https://sso.runtel.ru:8443/realms/runtel/protocol/openid-connect/token
api_url = https://sso.runtel.ru:8443/realms/runtel/protocol/openid-connect/userinfo
scopes = openid profile email groups
```

***Опционально!*** проверить настойки пользователей и разрешить/запретить ниже приведённые параметры
```bash
root@loki-grafana /etc/grafana > sed -n '511,526p' grafana.ini
#################################### Users ###############################
[users]
# Разрешить самостоятельную регистрацию
allow_sign_up = false
...
**Переазгрузка сервиса**
```bash
sudo systemctl restart grafana-server
sudo systemctl status grafana-server -l --no-pager
```

**ВАЖНО!** ***Все действия были выполнены без применения SSO Keycloak.***
