...
<details>
<summary>❗script-check #2 ❗</summary>
...
# 2. Проверка всех нод
for HOST in 192.168.87.60 192.168.87.61 192.168.87.62; do
    NODE_NAME=""
    case $HOST in
        192.168.87.60) NODE_NAME="app" ;;
        192.168.87.61) NODE_NAME="app2" ;;
        192.168.87.62) NODE_NAME="app3" ;;
    esac
...
```
### Альтернативно - через подключение к мастеру:
```sql
sudo -u postgres psql -p 5433 -h 192.168.87.60

-- Удалить
DROP DATABASE IF EXISTS rt_pbx_v2_ext;
DROP DATABASE IF EXISTS rt_pbx_v2_logging_ext;
DROP DATABASE IF EXISTS rt_pbx_v2_media_ext;
DROP DATABASE IF EXISTS rt_pbx_v2_stat_ext;

-- Создать
CREATE DATABASE rt_pbx_v2_ext WITH OWNER = rt_pbxx ENCODING = 'UTF8' LC_COLLATE = 'ru_RU.UTF-8' LC_CTYPE = 'ru_RU.UTF-8' TEMPLATE = template0;
CREATE DATABASE rt_pbx_v2_logging_ext WITH OWNER = rt_pbxx ENCODING = 'UTF8' LC_COLLATE = 'ru_RU.UTF-8' LC_CTYPE = 'ru_RU.UTF-8' TEMPLATE = template0;
CREATE DATABASE rt_pbx_v2_media_ext WITH OWNER = rt_pbxx ENCODING = 'UTF8' LC_COLLATE = 'ru_RU.UTF-8' LC_CTYPE = 'ru_RU.UTF-8' TEMPLATE = template0;
CREATE DATABASE rt_pbx_v2_stat_ext WITH OWNER = rt_pbxx ENCODING = 'UTF8' LC_COLLATE = 'ru_RU.UTF-8' LC_CTYPE = 'ru_RU.UTF-8' TEMPLATE = template0;

-- Права (опционально)
GRANT ALL PRIVILEGES ON DATABASE rt_pbx_v2_ext TO rt_pbxx;
GRANT ALL PRIVILEGES ON DATABASE rt_pbx_v2_logging_ext TO rt_pbxx;
GRANT ALL PRIVILEGES ON DATABASE rt_pbx_v2_media_ext TO rt_pbxx;
GRANT ALL PRIVILEGES ON DATABASE rt_pbx_v2_stat_ext TO rt_pbxx;
...
## Если требуется создать пользователя rt_pbxx и пароль для него
```sql
...
## Проверить инфо о пользователе
```sql
-- В том же подключении psql
\du rt_pbxx
...
FROM pg_shadow 
WHERE usename = 'rt_pbxx';
```
```bash
# Теперь попробуйте подключиться
sudo -u postgres psql -p 5433 -h 192.168.87.60 -U rt_pbxx -d postgres

# Или с явным указанием пароля
PGPASSWORD='VeryComplexPass123' psql -h 192.168.87.60 -p 5433 -U rt_pbxx -d postgres

# Проверьте без пароля
psql -h 192.168.87.60 -p 5433 -U rt_pbxx -d postgres


# Попробуйте разные варианты подключения

# 1. Через строку подключения
psql "postgresql://rt_pbxx:VeryComplexPass123@192.168.87.60:5433/postgres"

# 2. Через .pgpass
sudo -u postgres bash -c 'PGPASSFILE=/var/lib/postgresql/.pgpass psql -h 192.168.87.60 -p 5433 -U rt_pbxx -d postgres'

# 3. С логированием
PGPASSWORD='VeryComplexPass123' psql -h 192.168.87.60 -p 5433 -U rt_pbxx -d postgres -c "SELECT 1"
```

## Проверить метод аутентификации
```sql
-- Проверьте текущий метод шифрования паролей
SHOW password_encryption;

-- Проверить метод для конкретного пользователя
SELECT 
    usename,
    CASE 
        WHEN passwd LIKE 'SCRAM-SHA-256%' THEN 'scram-sha-256'
...
-- Если показывает 'md5', а не 'scram-sha-256', то:
ALTER USER rt_pbxx WITH PASSWORD 'VeryComplexPass123';
-- Это перешифрует пароль в текущем формате
```
```sql
-- Проверьте все пользователи и их методы аутентификации
SELECT 
    usename,
    CASE 
        WHEN passwd LIKE 'SCRAM-SHA-256%' THEN 'scram-sha-256'
...
1. **База данных инициализирована НЕПРАВИЛЬНО**
2. **Таблицы созданы, но с неправильными типами данных**
3. **Пользователь `root@runtel.ru` создан, но структура БД кривая**
...
```bash
# Подключись к БД
psql -h 192.168.87.60 -p 5432 -U postgres -d rt_pbx_v2_ext
...
### **Шаг 2: Проверить, все ли таблицы созданы**
```bash
# Сколько всего таблиц?
SELECT COUNT(*) FROM information_schema.tables 
WHERE table_schema = 'public';

# Сравни с ожидаемым (у тебя было 84 таблицы в выводе ранее)
...
Таблицы есть, но структура неправильная.

### **3. Проблема версий:**
Приложение ожидает одну структуру БД, а создалась другая.

## **Что делать:**

### **Вариант 1: Пересоздать БД (если можно)**
```bash
# 1. Удали и пересоздай БД
psql -h 192.168.87.60 -p 5432 -U postgres -c "DROP DATABASE IF EXISTS rt_pbx_v2_ext;"
psql -h 192.168.87.60 -p 5432 -U postgres -c "CREATE DATABASE rt_pbx_v2_ext OWNER rt_pbxx;"

# 2. Удали пользователя root@runtel.ru из других БД если нужно
psql -h 192.168.87.60 -p 5432 -U postgres -d pbxv2 \
  -c "DELETE FROM public.user WHERE email = 'root@runtel.ru';"

# 3. Перезапусти Ansible
ansible-playbook playbook-clust-test.yml --tags fill_db
```

### **Вариант 2: Проверить и исправить структуру**
```sql
-- Исправить тип поля event
...
## **Добавь в Ansible проверку структуры БД:**
...
```bash
# 1. Проверь тип поля event
psql -h 192.168.87.60 -p 5432 -U postgres -d rt_pbx_v2_ext -c "
...
# 3. Проверь конкретную проблемную таблицу
psql -h 192.168.87.60 -p 5432 -U postgres -d rt_pbx_v2_ext -c "
...
SELECT column_name, data_type, is_nullable 
FROM information_schema.columns 
WHERE table_name = 'domain_event_handling' AND column_name = 'event';
...
**Проблема не в создании пользователя, а в структуре БД!** Команда `system_init` либо:
1. Выполнилась не до конца
2. Использовала неправильную схему/миграции
3. Была прервана
