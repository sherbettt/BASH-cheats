...
server {
    ...
    # SSL-сертификаты (должны быть заранее получены)
    ssl_certificate /etc/letsencrypt/live/sso.runtel.ru/fullchain.pem;
    ssl_certificate_key /etc/letsencrypt/live/sso.runtel.ru/privkey.pem;
    ssl_protocols TLSv1.2 TLSv1.3;

    # Проксирование на Keycloak
    location / {
        proxy_pass https://192.168.46.16:8443;
        proxy_set_header Host $host;
        proxy_set_header X-Real-IP $remote_addr;
        proxy_set_header X-Forwarded-For $proxy_add_x_forwarded_for;
