...
На машины ***Container 273 (keycloackd)*** и ***Container 272 (keycloack)*** потребуется залить дамп БД с машины ***Container 172 (keycloak)***. 
 ...
 Обнаружена проблема на машине ***Container 172 (keycloak)***: сервис пишет данные в свою собственную БД. Требуется исправить:
- Закоментировать строки с PostgreSQL;
- выполнить Export БД Keycloak в файл;
- поправить в этом файле параметры для PostgreSQL БД;
- выполнить Import из файла.

1) В рамках наших конфигураций, управляющий скрипт Keycloak находится в ***`/opt/keycloak/bin/`***.
    ```bash
    # экспорт realm с названием "master"
    root@keycloack /opt/keycloak/bin # ./kc.sh export --file /tmp/realm.file

    # экспорт realm с названием "runtel"
    ./kcadm.sh config credentials --server https://sso.runtel.ru:8443/ --realm runtel --user admin --password 3J4OQ52p2qRAgaDG
    ```
2) идём в https://sso.runtel.ru:8443 (нужно попасть под оригинальным keycloak, проверяем через F12 в браузере), т.е. не забыть закоментить дублирвоанные машины в /etc/hosts; т.к. оригинальный Keycloak по DHCP подключен к ProxMoX, то альтерантивно можно использовать: https://sso.runtel.ru:8443/admin/master/console/. 
3) Слева сверху выбрать Realm = runtel; Configure -> Realm Setting -> выпадающее меню Actions -> Partial export. Фактически получим такой же файлик, что и в п.1.
