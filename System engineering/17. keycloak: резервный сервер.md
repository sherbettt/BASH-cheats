...
1) Вначале нужно войти в realm = runtel.
    ```bash
    /opt/keycloak/bin # ./kcadm.sh config credentials \
    --server https://sso.runtel.ru:8443/ \
    --realm master \
    --user admin \
    --password 3J4OQ52p2qRAgaDG
...
2) В рамках наших конфигураций, управляющий скрипт Keycloak находится в **/opt/keycloak/bin/**.
    ...
    ```
3) идём в https://sso.runtel.ru:8443 (нужно попасть под оригинальным keycloak, проверяем через F12 в браузере), т.е. не забыть закоментить дублирвоанные машины в /etc/hosts; т.к. оригинальный Keycloak по DHCP подключен к ProxMoX, то альтерантивно можно использовать: https://sso.runtel.ru:8443/admin/master/console/. 
4) Слева сверху выбрать Realm = runtel; Configure -> Realm Setting -> выпадающее меню Actions -> Partial export. Фактически получим такой же файлик, что и в п.1.
