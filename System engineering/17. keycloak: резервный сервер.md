...
6) После успешного применения, создать дамп PostgreSQL и кинуть на мастер ноду pg* машины, в нашем случае это пока 192.168.45.201.
7) Перейти на 
8) На всех нодах, начиная с master, удалить текущую БД с названием "runtel" полностью и развернуть имеющийся дамп.
