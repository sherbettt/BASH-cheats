...
В рамках наших конфигураций, управляющий скрипт Keycloak находится в ***`/opt/keycloak/bin/`***.

идём в https://sso.runtel.ru:8443 (нужно попасть под оригинальным keycloak, проверяем через F12 в браузере), т.е. не забыть закоментить дублирвоанные машины в /etc/hosts.
