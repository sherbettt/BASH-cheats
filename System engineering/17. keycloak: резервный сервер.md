...
### 2. Тестирование по одной машине.
  1. На машине 192.168.87.2 прописываем в *`/etc/hosts`* наши новые, дуплицированные от оригинала - *keycloak* машины.
     ```text
     # SSO, keycloak duplicates
     192.168.46.16  keycloak
     192.168.46.17  keycloakd
     ```
  3. Заглушаем ***Container 172 (keycloak) on node 'prox4'*** , на ноуте в /etc/hosts указываем адрес
