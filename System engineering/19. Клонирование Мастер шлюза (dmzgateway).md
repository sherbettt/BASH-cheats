...
<details>
<summary>❗keepalived.conf на dmzgateway1❗</summary>
...
# === ЛОКАЛЬНЫЙ ШЛЮЗ для сети 192.168.87.0/24 ===
vrrp_instance VI_VMBR0 {
    ...
    # Notify скрипты - выполняются при смене состояния
...
# === ШЛЮЗ для DMZ сети 192.168.46.0/24 ===
vrrp_instance VI_DMZNET {
    ...
    unicast_peer {
        192.168.46.252   # dmzgateway2
        192.168.46.251   # dmzgateway3
    }
...
# === ШЛЮЗ для PG сети 192.168.45.0/24 ===
vrrp_instance VI_PGNET {
    ...
    unicast_peer {
        192.168.45.252   # dmzgateway2
        192.168.45.251   # dmzgateway3
    }
...
# === TELEPORT (отдельный VRRP instance с УНИКАЛЬНЫМ ID) ===
vrrp_instance VI_TELEPORT {
    state BACKUP                            # текущая нода BACKUP
    interface eth0                          # Интерфейс для VRRP
    virtual_router_id 41                    # УНИКАЛЬНЫЙ ID (не 61, 62 или 63!)
    priority 100                            # Приоритет: 150 (мастер), 120, 100 и ниже (бэкапы)
    advert_int 1                            # Частота объявлений (сек)
    #nopreempt                              # Если ВКЛ., то nopreempt запрещает ноде автоматически становиться MASTER, даже если её приоритет выше, чем у текущего MASTER'а.

    # Notify скрипты
    notify_master "/etc/keepalived/master.sh VI_TELEPORT"
    notify_backup "/etc/keepalived/backup.sh VI_TELEPORT"
    notify_fault "/etc/keepalived/backup.sh VI_TELEPORT"
...
Для сетей **DMZ** и **PG** применён режим `**unicast_peer**` , т.к. со временем обнаружилась проблема перехвата MASTER сразу на двух машинах (dmzgateway1 и dmzgateway2 или dmzgateway2 и dmzgateway3).

</details>


<details>
<summary>❗keepalived.conf на dmzgateway2 (похоже на dmzgateway3)❗</summary>

```cfg
global_defs {
    router_id dmzgateway2
    vrrp_debug_level 2
}
...
vrrp_instance VI_TELEPORT {
    ...
    interface eth0                          # Интерфейс для VRRP
    virtual_router_id 41                    # УНИКАЛЬНЫЙ ID (не 61, 62 или 63!)
    priority 95                             # Приоритет: 150 (мастер), 120, 100 и ниже (бэкапы)
    advert_int 1
...
### 3.1. Notify скрипты для логирования состояний
...
```bash
#!/bin/bash
echo "[$(date)] BACKUP: $HOSTNAME - $@" >> /var/log/keepalived-state.log
echo "Keepalived: $HOSTNAME перешел в BACKUP состояние ($@)" | write root
```
```bash
#!/bin/bash
echo "[$(date)] MASTER: $HOSTNAME - $@" >> /var/log/keepalived-state.log
echo "Keepalived: $HOSTNAME перешел в MASTER состояние ($@)" | write root
```
...
### 3.2. Тест с реальными параметрами
```bash
# Для VI_VMBR0
/etc/keepalived/master.sh VI_VMBR0
/etc/keepalived/backup.sh VI_VMBR0

# Для VI_DMZNET
/etc/keepalived/master.sh VI_DMZNET  
/etc/keepalived/backup.sh VI_DMZNET

# Для VI_PGNET
/etc/keepalived/master.sh VI_PGNET
/etc/keepalived/backup.sh VI_PGNET

# С интерфейсом и VIP
/etc/keepalived/master.sh VI_VMBR0 eth0 192.168.87.2
/etc/keepalived/backup.sh VI_VMBR0 eth0 192.168.87.2

/etc/keepalived/master.sh VI_DMZNET eth1 192.168.46.1
/etc/keepalived/backup.sh VI_DMZNET eth1 192.168.46.1

/etc/keepalived/master.sh VI_PGNET eth2 192.168.45.1
/etc/keepalived/backup.sh VI_PGNET eth2 192.168.45.1
```
...
journalctl -u keepalived -f | grep -E "VMBR0|DMZNET|PGNET|TELEPORT"
