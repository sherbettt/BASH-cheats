...
ansible gateways -m copy -a "content='group cluster {
    ...
    };       # <- в некоторых случаях символ ";" не нужен
...
```bash
ansible gateways -m shell -a "openssl req -x509 -newkey rsa:4096 -keyout /etc/csync2_ssl_key.pem -out /etc/csync2_ssl_cert.pem -days 3650 -nodes -subj "/CN=dmzgateway1""
```
```bash
