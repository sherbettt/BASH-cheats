...
1. Все команды выполняются с управляющей машины (192.168.87.74)
2. Используется инвентарный файл `hosts` с группами gateways и proxmox
3. Для аутентификации используется пароль root (`-k` флаг)
4. Ключ генерируется один раз на dmzgateway1 и распространяется на остальные узлы
