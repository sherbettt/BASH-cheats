...
vrrp_instance VI_VMBR0 {
    ...
    virtual_router_id 61   # Должен быть одинаковым в кластере
    ...
    authentication {
		auth_type NONE
        #auth_type PASS
        #auth_pass "0"      # Явное указание "нулевой" аутентификации
    }                      
    virtual_ipaddress {
...
Однако, может получится так, что после перезагрузки трёх шлюзов VIP могут разделиться по машинам, что неправильно и ломает маршруты.
Для этого нужно закомментировать параметр *`nopreempt`* и поменять параметр *`priority`*. Получится своеобразный костыль, но рабочий.
...
```bash
    ...
    inet 192.168.87.2/24 scope global secondary eth0
    inet 192.168.46.253/24 scope global eth1
    inet 192.168.46.1/24 scope global secondary eth1
    inet 192.168.45.253/24 scope global eth2
    inet 192.168.45.1/24 scope global secondary eth2
```
