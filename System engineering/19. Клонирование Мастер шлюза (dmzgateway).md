...
### 3. Установка пакета csync2 на все шлюзы
```bash
...
### 4. Создание конфигурационных каталогов
```bash
...
### 5.1. Генерация ключа на dmzgateway1 и копирование на остальные узлы
```bash
...
### 5.2. Генерация ключа локальной машине (192.168.87.74) и копирование на остальные узлы
```bash
...
### 6. Создание конфигурационного файла `/etc/csync2/csync2.cfg`
...
### 6 [ALT]. Создание конфигурационного файла `/etc/csync2/csync2.cfg` с помощью jinja
...
### 7. Создание `/etc/systemd/system/csync2.service` юнита
```bash
...
#### Можно руками на мастер ноде руками создать и руками разложить по нодам:
```bash
