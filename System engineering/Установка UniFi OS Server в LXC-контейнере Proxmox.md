...
Возникнет ошибка "Bad FW Image Signature", потому что прошивка не прошла проверку подписи. Это стандартная защита Ubiquiti, и её можно обойти. 
Причина в том, что запуская **`syswrapper.sh upgrade2 &`**, он по умолчанию пытается скачать прошивку из интернета (без VPN для РФ) и проверяет цифровую подпись.


```bash
# Остановите фоновый процесс (если он ещё висит)
# Нажмите Ctrl+C, если видите процесс

# Запустите обновление с локальным файлом
upgrade /tmp/fwupdate.bin

# Или если хотите с другим именем файла:
upgrade /tmp/UniFi_firmware_8.6.11_for_U7-Mesh
```

#### Если upgrade не сработает — используйте sysupgrade
На прошивках на основе OpenWrt/LEDE есть команда sysupgrade:
```bash
# Обновление с сохранением настроек
sysupgrade -n /tmp/fwupdate.bin
```
Флаг `-n` означает "без сохранения настроек" (чистая установка). Если хотите сохранить настройки, уберите `-n`.
...
# Обновление точек доступа UniFi через локальный HTTP-сервер

## Введение

При работе с оборудованием Ubiquiti в условиях географических ограничений (например, в РФ) возникает проблема: точки доступа не могут скачать обновления напрямую с официальных серверов Ubiquiti.

---

## Подготовка: скачивание прошивок на сервер

Прежде всего необходимо скачать актуальные прошивки для ваших моделей точек доступа с официального сайта Ubiquiti.

### Структура файлов на сервере

После скачивания файлов в каталоге `/root/programs/` у нас оказались:

```bash
root@UniFi-hotspot ~/programs
13:40:13 # ll
total 2.0G
drwxr-xr-x  2 root root 4.1k Jun 22 13:40 ./
drwx------ 11 root root 4.1k Jun 22 12:16 ../
-rwxr-xr-x  1 root root 875M Jun 17 06:04 24e0-linux-x64-5.1.15-...-x64*
-rw-r--r--  1 root root  65M Jun 17 06:11 BZ.ipq53xx_8.6.11+18870.260526.1140.bin
-rwxr-xr-x  1 root root 881M Jun 19 13:40 UniFi-OS-Server_5.1.19_Linux_x64.19-x64*
-rw-r--r--  1 root root  47M May 26 14:43 UniFi_firmware_8.6.11_U7-Mesh.bin
-rw-r--r--  1 root root  65M May 26 14:43 UniFi_firmware_8.6.11_U7-Pro.bin
```

**Пояснение файлов:**
- `UniFi_firmware_8.6.11_U7-Mesh.bin` — прошивка для U7 Mesh (47 МБ)
- `UniFi_firmware_8.6.11_U7-Pro.bin` — прошивка для U7 Pro (65 МБ)
- Оба файла имеют тип `HIT archive data` — это стандартный формат прошивок Ubiquiti

### Процесс скачивания

```bash
# Скачиваем прошивку для U7 Mesh (модель U7-Mesh, 47 МБ)
wget https://dl.ui.com/unifi/firmware/G7LR/8.6.11.18870/BZ2.ipq53xx_8.6.11+18870.260526.1140.bin

# Скачиваем прошивку для U7 Pro (модель U7-Pro, 65 МБ)
wget https://dl.ui.com/unifi/firmware/U7PRO/8.6.11.18870/BZ.ipq53xx_8.6.11+18870.260526.1140.bin
```

> **Важно:** Скачивание выполняется на сервере через VPN, так как `dl.ui.com` может быть недоступен напрямую из РФ. После скачивания файлы переименовываются в человеко-читаемый формат для удобства использования.

---

## Запуск HTTP-сервера

Для того чтобы точки доступа могли скачать прошивку с сервера, запускаем простой HTTP-сервер:

```bash
root@UniFi-hotspot ~/programs
13:40:20 # python3 -m http.server 8000
Serving HTTP on 0.0.0.0 port 8000 (http://0.0.0.0:8000/) ...
```

**Что это даёт:**
- Сервер становится доступен по адресу `http://192.168.87.153:8000/`
- Все файлы в папке `/root/programs/` становятся доступны для скачивания
- Точки доступа могут получить прошивку по прямой ссылке

---

## Процесс обновления через веб-интерфейс

### Шаг 1: Получение ссылки на прошивку

Из логов HTTP-сервера видно, как точка доступа запрашивает файл:

```
192.168.97.220 - - [22/Jun/2026 13:42:46] "GET /UniFi_firmware_8.6.11_U7-Pro.bin HTTP/1.1" 200 -
```

**Ссылка для U7 Pro:**
```
http://192.168.87.153:8000/UniFi_firmware_8.6.11_U7-Pro.bin
```

**Ссылка для U7 Mesh:**
```
http://192.168.87.153:8000/UniFi_firmware_8.6.11_U7-Mesh.bin
```

### Шаг 2: Обновление через веб-интерфейс

1. Зайдите в веб-интерфейс UniFi OS Server: `https://192.168.87.153:11443/`
2. Перейдите в раздел **Devices** (Устройства)
3. Выберите точку доступа, которую нужно обновить
4. В правой панели откройте вкладку **Settings** (Настройки)
5. Найдите раздел **Manual Firmware Update** (Пользовательское обновление)
...
Логи показывают процесс обращения точек доступа к серверу:

```
192.168.92.69 - - [22/Jun/2026 13:41:12] code 404, message File not found
192.168.92.69 - - [22/Jun/2026 13:41:12] "GET /UniFi_firmware_8.6.11_for_U7-Mesh HTTP/1.1" 404 -
192.168.92.69 - - [22/Jun/2026 13:41:23] "GET / HTTP/1.1" 200 -
192.168.97.220 - - [22/Jun/2026 13:42:46] "GET /UniFi_firmware_8.6.11_U7-Pro.bin HTTP/1.1" 200 -
```

**Анализ логов:**
1. **Запрос с ошибкой 404** (IP 192.168.92.69) — точка доступа пыталась скачать файл по старому имени, которое было удалено
2. **Успешный запрос** (IP 192.168.97.220) — после исправления имени файла точка доступа успешно скачала прошивку
3. **Код 200** — файл найден и передан точке доступа

---

## ✅ Преимущества метода

1. **Централизованное хранение** — все прошивки хранятся в одном месте на сервере
2. **Не зависит от внешних серверов** — обновления работают даже при отсутствии доступа в интернет
3. **Простота использования** — не нужно подключаться к каждой точке доступа по SSH
4. **Массовое обновление** — можно обновить несколько точек одновременно через веб-интерфейс
5. **Прозрачность процесса** — логи HTTP-сервера показывают, какая точка и когда скачала прошивку
...
curl -I http://192.168.87.153:8000/UniFi_firmware_8.6.11_U7-Pro.bin
...
Метод обновления через локальный HTTP-сервер показал свою эффективность. Он позволяет обновлять точки доступа UniFi в условиях, когда прямое подключение к серверам Ubiquiti невозможно. Все операции выполняются через веб-интерфейс, что значительно упрощает процесс и делает его доступным даже для администраторов без глубоких знаний командной строки.
