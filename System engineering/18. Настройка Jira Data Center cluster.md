...
4. Восстановление с принудительным режимом
```bash
sudo -u postgres pg_restore -Fc -d jira -j 2 --clean --if-exists --verbose /var/backups/postgresql/jira_db.dump
```
Ключевые параметры:
- `--clean` - удалять существующие объекты
- `--if-exists` - избегать ошибок при удалении
- `-j 2` - умеренный параллелизм (меньше ошибок)

 5. Альтернативный вариант восстановления
```bash
sudo -u postgres pg_restore -Fc -d jira -j 1 --no-owner --no-privileges /var/backups/postgresql/jira_db.dump
```

 6. Если проблема сохраняется, попробуем альтернативный подход:
```bash
