...
pg_dump -U postgres -Fc -d jira -f /var/backups/postgres/jira_db.dump
```
где:
- `-U postgres` — явно указывает, что нужно подключиться от имени пользователя postgres;
```bash
pg_dump -U postgres -W -Fc -d jira -f /var/backups/postgres/jira_db.dump
```
где:
- `-W` — явно указывать пароль
