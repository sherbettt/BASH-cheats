...
```properties
# jira-new (192.168.87.219)
# /var/atlassian/application-data/jira/hazelcast.properties
hazelcast.network.tcpip.members=192.168.46.2,192.168.46.3,192.168.46.4,192.168.87.219
hazelcast.interface=192.168.45.254  # Используем eth1 (внутренний интерфейс)
hazelcast.socket.bind.any=false     # Жёсткая привязка к интерфейсу
hazelcast.group.name=jira-cluster   # одинаковое на всех узлах
hazelcast.group.password=runtelorg
#hazelcast.interface=192.168.87.219  # Слушаем на eth0 (основной интерфейс)
hazelcast.logging.type=log4j
hazelcast.phone.home.enabled=false
hazelcast.initial.min.cluster.size=4  # Минимум узлов для работы
```
```properties
# jira-node (192.168.46.3)
hazelcast.network.tcpip.members=192.168.46.2,192.168.46.3,192.168.46.4,192.168.87.219
hazelcast.group.name=jira-cluster
hazelcast.group.password=runtelorg
hazelcast.interface=192.168.46.3  # Слушаем на основном интерфейсе
...
```
```properties
# jira-cluster (192.168.46.2)
hazelcast.network.tcpip.members=192.168.46.2,192.168.46.3,192.168.46.4,192.168.87.219
hazelcast.group.name=jira-cluster
hazelcast.group.password=runtelorg
hazelcast.interface=192.168.46.2  # Слушаем на основном интерфейсе
hazelcast.socket.bind.any=false
hazelcast.logging.type=log4j
hazelcast.phone.home.enabled=false
hazelcast.initial.min.cluster.size=4  # Минимум узлов для работы
```
```properties
# jira-node2 (192.168.46.4)
hazelcast.network.tcpip.members=192.168.46.2,192.168.46.3,192.168.46.4,192.168.87.219
hazelcast.group.name=jira-node2
hazelcast.group.password=runtelorg
hazelcast.interface=192.168.46.4  # Слушаем на основном интерфейсе
hazelcast.socket.bind.any=false
hazelcast.logging.type=log4j
hazelcast.phone.home.enabled=false
hazelcast.initial.min.cluster.size=4  # Минимум узлов для работы
```
Логи ищи командой `grep -A 10 "Hazelcast" /var/atlassian/application-data/jira/log/atlassian-jira.log | ccze -A`

Проверка связи всех нод одним скриптом:
```sh
# Проверка связи с другими узлами
for node in 192.168.46.2 192.168.46.3 192.168.46.4 192.168.87.219; do
...
<br/> Мониторинг **https://jira.runtel.ru/plugins/servlet/cluster-monitoring**
<br/> Инструменты для устранения неисправностей и поддержки **https://jira.runtel.ru/plugins/servlet/troubleshooting/view?source=notification&healthCheck=clusterLockHealthCheck**
<br/> Индексирвоание **https://jira.runtel.ru/secure/admin/jira/IndexAdmin.jspa?reindexTime=0**
<br/> Консоль скриптов **https://jira.runtel.ru/secure/admin/InsightPluginInitScriptConsole.jspa**
<br/>
<br/>
...
В файле **`/etc/crontab`** на **prox4,pmx5 и pmx6** добавить строку внизу:
```
*/5 *	* * * 	root rsync -avP root@192.168.87.3:/stg/8tb/share/ /stg/8tb/share/
```
т.е. каждые 5 минут будет выполняться команда rsync с указанными параметрами:
1. **`*/5 * * * *`** – Расписание (schedule):  
   - **`*/5`** – Каждые 5 минут.  
   - **`*`** – Любой час.  
   - **`*`** – Любой день месяца.  
   - **`*`** – Любой месяц.  
...
2. **`root`** – Пользователь, от которого выполняется команда.  

3. **`rsync -avP root@192.168.87.3:/stg/8tb/share/ /stg/8tb/share/`** – Сама команда:  
   ...
   - **`-P`** – Прогресс и докачка (аналог `--partial --progress`).  
   - **`root@192.168.87.3:/stg/8tb/share/`** – Источник (удалённый сервер `192.168.87.3`, папка `/stg/8tb/share/`).  
   - **`/stg/8tb/share/`** – Целевая директория (локальная).  
<br/>


## 7. BackUP на машину "Container 101 (backup)" (192.168.87.99)
Учитывая, что у нас есть плавающий IP=192.168.87.3, прикреплённый к "железным" машинам (pmx5, pmx6, prox4), требуется делать бэкап с них на машину backup.
<details>
...
Создаём простенький автоскрипт:
```bash
#!/bin/bash
# /root/automate/rdiff-backup-jira.sh
echo "========================================="
echo "Текущая дата: $(date +"%F")"
echo "Время начала: $(date -R)"
echo "------------------"

# Основная команда создания бэкапа (старый синтаксис)
/usr/bin/rdiff-backup --no-new  --remote-schema 'ssh -C %s rdiff-backup --server' -v5 \
    root@192.168.87.3::/stg/8tb/share/ /backups/jira-data/

# Очистка старых бэкапов (старый синтаксис)
echo "Удаление старых бекапов, оставляем последние 12-ть: "
/usr/bin/rdiff-backup --no-new --remove-older-than 12B --force /backups/jira-data/

echo "Бекап завершён"
echo "Время завершения: $(date -R)"
echo "=========================================="
```

Посмотреть текущее UTC время:
```bash
date -u
```
Посчитать московское время:
```bash
date -d 'TZ="Europe/Moscow" now'
```
Пример проверки:
```bash
# Какое время будет в Москве, когда на сервере (UTC) 23:00?
date -d '23:00 UTC today + 3 hours' +"%H:%M %Z"
```
**Лучше оставить сервер в UTC и делать поправку в cron - это стандартная практика для серверов.**
...
```
```bash
0 23 * * * /root/automate/rdiff-backup-jira.sh
```
<br/>
