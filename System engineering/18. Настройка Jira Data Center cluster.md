...
sudo -u postgres psql -c "CREATE USER jira WITH NOLOGIN;"
...
```bash
sudo -u postgres psql -d jira -c "ALTER USER jira WITH LOGIN;"
...
#### 1. Добавляем базу в конфигурацию Patroni
Отредактируйте `/etc/patroni.yml` на всех узлах:
...
#### 2. Перезагружаем конфигурацию Patroni
```bash
# На мастере:
patronictl reload-config pgcluster
...
1. **Попробуйте подключиться**:
   ```bash
   psql -h vip_address -U jira -d jira
   ```
...
3. **Мониторинг**:
   ```bash
   patronictl list
   watch -n 1 'psql -h localhost -U postgres -c "SELECT application_name, state, sync_state FROM pg_stat_replication;"'
   ```

Если таблицы остаются пустыми на репликах, возможно, потребуется:
1. Полное повторное восстановление с включенной логической репликацией
2. Проверка места на диске на репликах
3. Анализ логов Patroni (`journalctl -u patroni -n 100`)
