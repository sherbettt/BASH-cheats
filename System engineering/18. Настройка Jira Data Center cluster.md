...
### 3. Восстановить БД.
1. Сначала удалим существующую БД `jira`:

```bash
sudo -u postgres psql -c "DROP DATABASE IF EXISTS jira;"
```

2. Убедимся, что пользователь `jira` существует (если нет - создадим):
...
3. Создадим БД с правильным владельцем:

```bash
sudo -u postgres psql -c "CREATE DATABASE jira WITH OWNER jira;"
```

4. Восстановим данные из дампа (убедитесь, что Patroni работает):

```bash
sudo -u postgres pg_restore -Fc -d jira -j 4 /var/backups/postgresql/jira_db.dump
```
...
Важные замечания:
- Не нужно останавливать postgresql.service - это просто заглушка в системе с Patroni
- Фактически PostgreSQL управляется через patroni.service, который должен быть активен
- Если восстановление прерывается (как в вашем предыдущем опыте), попробуйте без параллелизма (`-j 1`)

Если восстановление не удается, можно попробовать с дополнительными параметрами:
```bash
sudo -u postgres pg_restore -Fc -d jira -j 1 --verbose /var/backups/postgresql/jira_db.dump
```
