...
<p align="center">
  <img src="https://github.com/sherbettt/BASH-cheats/blob/main/images/UniFI-OS_SSH-hotsopt_5.1.19_console5.png" alt="UniFI-OS_SSH-hotsopt_5.1.19_console5.png">
</p>
...
## Восстановление через TFTP (для "кирпичей" и критических случаев)

Это тот самый метод, который использует ваш настроенный TFTP-сервер. Важно: точки доступа UniFi ожидают файл в формате **`.bin`** именно по этому протоколу .

### Как это выглядит в реальной работе:

1.  **Подготовьте файл прошивки** (например, `BZ.qca956x.v3.7.58.bin`) и положите его в вашу TFTP-директорию `/srv/tftp/` .
2.  **Переведите точку доступа в режим TFTP-восстановления**. Для этого:
    *   Отключите питание AP.
    *   **Зажмите Reset-кнопку** и, не отпуская, подайте питание.
    *   Держите кнопку, пока светодиод не начнет мигать в последовательности **белый > синий > выключен** (или просто быстро замигает белым и синим) . Это сигнал, что устройство готово принять прошивку.
3.  **Временно настройте IP на своем компьютере** в подсети `192.168.1.0/24`. Точка доступа в этом режиме получает статический адрес `192.168.1.20` .
4.  **Отправьте прошивку** с сервера или ноутбука:
    ```bash
    # Команда с вашего настроенного клиента tftp-hpa
    tftp 192.168.1.20 -c put /srv/tftp/BZ.qca956x.v3.7.58.bin
    ```
    В интерактивном режиме это выглядит так: `put BZ.qca956x.v3.7.58.6385.170508.0957.bin` . После завершения передачи устройство автоматически перезагрузится с новой прошивкой .

#### о форматах файлов

*   **`.bin`**: Используется для обновления прошивки большинства точек доступа UniFi AP, как через TFTP, так и по HTTP/HTTPS .
*   **`.tar`**: Используется для прошивки **USG (UniFi Security Gateway)**. В этом случае прошивка загружается и устанавливается по HTTP/HTTPS .
