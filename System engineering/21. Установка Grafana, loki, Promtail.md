...
Проверка меток на машине **`Loki+Promtail+Grafana`**:
```bash
# Все доступные label names
logcli labels --addr=http://localhost:3100

# Посмотреть значения для конкретных меток
...
Проверка логов на машине с **`Loki+Promtail+Grafana`**:
```bash
logcli query '{job="*"}' --addr=http://localhost:3100
logcli query '{service_name="*"}' --addr=http://localhost:3100

logcli series --addr=http://localhost:3100 '{}'   # просмотр ВСЕХ логов со ВСЕХ машин
...
# Проверим логи с сервиса/машины runtel-logs-msk-vps1
logcli query --addr=http://localhost:3100 '{service_name=~"runtel-logs-msk-vps1"}' --limit=10
...
Использование регулярных выражений в метках
```bash
# Все логи со всех машин (аналог 'tail -f' для всей инфраструктуры)
logcli query '{job=~".+"}' --addr=http://localhost:3100 --limit=50

# Логи с машин, содержащих в имени "60" или "209"
logcli query '{job=~".*-(60|209)"}' --addr=http://localhost:3100 --limit=20

# Логи с машин, имена которых начинаются на "runtel"
logcli query '{job=~"runtel.+"}' --addr=http://localhost:3100 --limit=30
```
Определение полей
```bash
logcli detected-fields --addr="http://localhost:3100" '{service_name=~"runtel-logs-msk-vps1"}'
logcli detected-fields --addr="http://localhost:3100" '{level=~"ERROR"}'
logcli detected-fields --addr="http://localhost:3100" '{filename=~"/var/log/auth.log"}'
