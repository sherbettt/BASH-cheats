...
if /usr/bin/rsync -avz --progress /var/backups/postgresql/*_backup_${BACKUP_DATE}.sql.gz postgres@${BACKUP_SERVER}:${BACKUP_PATH}; then
...
# Удаляем старые бэкапы (старше 1 дня)
/usr/bin/find /var/backups/postgresql/ -name "*_backup_*.sql.gz" -mtime +1 -delete
