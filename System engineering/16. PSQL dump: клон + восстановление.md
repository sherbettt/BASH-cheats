...
progress_bar() {
    ...
    local space_occupied=$(( (space_available * $2) / 100 ))
    local space_left=$(( space_available - space_occupied ))
    
    printf "\r["
    printf "%${space_occupied}s" | tr ' ' '='
    printf "%${space_left}s" | tr ' ' ' '
    printf "] %3d%%" "$2"
}

# Функция для анимированного прогресса
spinner() {
    local pid=$1
    local delay=0.1
    local spinstr='|/-\'
...
echo "------"
echo "Создание Backup БД для текущей(его) реплики(лидера) на $(hostname)"
echo "Дата бэкапа: $(date +'%d.%m.%Y')"
echo "Backup server: 192.168.87.99:/var/backups/postgresql/$(hostname)/"

# Бэкап только Jira и Keycloak
cd /tmp
DATABASES=("jira" "jira_old" "keycloak")

for i in "${!DATABASES[@]}"; do
    DB=${DATABASES[$i]}
    BACKUP_FILE="/var/backups/postgresql/${DB}_backup_$(date +'%d.%m.%Y').sql"
    
    echo -n "Создание дампа ${DB}... "
    # Запускаем дамп в фоне
    /usr/bin/pg_dump $DB > $BACKUP_FILE 2>/dev/null &
    DUMP_PID=$!
    
    # Показываем спиннер во время выполнения
    spinner $DUMP_PID &
    SPINNER_PID=$!
...
# 2. Прогресс для rsync с использованием pv
echo "Копирование на backup сервер..."
BACKUP_FILES=(/var/backups/postgresql/*_backup_*.sql.gz)
TOTAL_FILES=${#BACKUP_FILES[@]}
COPIED=0

for file in "${BACKUP_FILES[@]}"; do
    FILENAME=$(basename "$file")
    echo -n "Копирование $FILENAME... "
    
    # Используем pv для отображения прогресса, если установлен
    if command -v pv >/dev/null 2>&1; then
        FILESIZE=$(stat -c%s "$file")
        pv -pet -s $FILESIZE "$file" | ssh postgres@192.168.87.99 "cat > /var/backups/postgresql/$(hostname)/$FILENAME"
    else
        # Альтернатива без pv
        rsync --progress "$file" postgres@192.168.87.99:/var/backups/postgresql/$(hostname)/ 2>&1 | \
        while read line; do
            if [[ $line =~ ([0-9]+)% ]]; then
                printf "\rКопирование $FILENAME... %3d%%" "${BASH_REMATCH[1]}"
            fi
        done
        echo
    fi
    ...
    ((COPIED++))
    printf "\rФайлов скопировано: %d/%d\n" "$COPIED" "$TOTAL_FILES"
done

# 3. Альтернативный вариант с простым прогрессом
...
for file in "${BACKUP_FILES[@]}"; do
    ...
    (rsync "$file" postgres@192.168.87.99:/var/backups/postgresql/$(hostname)/ > /dev/null 2>&1 && echo "✓") || echo "✗"
done

# Проверка доступности сервера
echo -n "Проверка доступности backup сервера... "
if ping -c 2 -W 2 192.168.87.99 >/dev/null 2>&1; then
    echo "✓"
else
    echo "✗"
    exit 1
...
echo -n "Удаление старых бэкапов... "
find /var/backups/postgresql/ -name "*_backup_*.sql.gz" -mtime +1 -delete
echo "✓"

echo "Локальные бэкапы созданы:"
ls -alhF /var/backups/postgresql/
```
</details>
