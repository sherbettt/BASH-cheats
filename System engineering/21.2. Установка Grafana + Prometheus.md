...
Ссылки с подсказками:
1. [Configure Tempo](https://github.com/grafana/tempo/blob/main/docs/sources/tempo/configuration/_index.md)
2. [grafana.com/docs/tempo/](https://grafana.com/docs/tempo/latest/configuration/)
...
## 1 ==========================
server:
  http_listen_port: 3200	          # HTTP порт для API и веб-интерфейса
  grpc_listen_port: 9095	          # gRPC порт для внутренней коммуникации
  ...
  receivers:
    ...
    jaeger:
      protocols:
        ...
        thrift_compact: # Jaeger Thrift compact
          endpoint: "0.0.0.0:6831"
        thrift_binary: # Jaeger Thrift binary
          endpoint: "0.0.0.0:6832"

    zipkin:
      endpoint: "0.0.0.0:9411"       # Zipkin протокол
    
    kafka:
      # Для приема из Apache Kafka

## 3 ==========================
ingester:
  max_block_duration: 5m              # Длительность блоков трассировок
  trace_idle_period: 10s              # Период бездействия трассировки
  max_block_bytes: 100_000_000        # Максимальный размер блока (100MB)
...
compactor:
  compaction:
    ...
    max_block_bytes: 100_000_000      # Максимальный размер блока после компрессии
    block_retention: 24h              # Время хранения трассировок (24 часа)
    compacted_block_retention: 1h     # Время хранения сжатых блоков

    # vertical_compaction_strategies:
    #   - size: 512MB                 # Стратегия сжатия по размеру
    #     stages: 2
    #   - size: 1GB
    #     stages: 2

## 5 ==========================
# Локальная файловая система (для тестирования)
storage:
  trace:
    ...
    # pool:
    #   max_workers: 100              # Макс. воркеров для записи
    #   queue_depth: 10000            # Глубина очереди

# Для продакшена лучше использовать S3/GCS:
#storage:
#  trace:
#    backend: s3                       # Используем S3
#    s3:
#      bucket: tempo-traces            # Имя бакета
#      endpoint: s3.amazonaws.com      # или minio.example.com
#      access_key: "AKIAIOSFODNN7EXAMPLE"
#      secret_key: "wJalrXUtnFEMI/K7MDENG/bPxRfiCYEXAMPLEKEY"
#      region: us-east-1
#      # insecure: false               # Для self-signed certs
#      # part_size: 5242880            # Размер части для multipart upload
...
## 6 ==========================
search_enabled: true                  # Включить поиск трассировок
search:
  external_endpoints:
    - http://localhost:3100           # Loki для связанных логов

  # query_timeout: 10s                # Таймаут поиска
  # concurrent_jobs: 100              # Макс. concurrent запросов
  
  # Провайдер поиска (можно выбрать):
  # backend: tsdb                     # Встроенный (рекомендуется)
  # tsdb:
  #   hedge_requests_at: 2s           # Оптимизация запросов
...
tracing:
  enabled: true
  sampling_fraction: 0.1              # Отправлять 10% трассировок
  jaeger:
    collector:
      endpoint: localhost:14250       # Куда отправлять трассировки Tempo
```
</details>
