...
global:
  scrape_interval: 15s      # Каждые 15 секунд собираем метрики
  evaluation_interval: 15s  # Каждые 15 секунд проверяем правила
    ...
    static_configs:
      - targets: ['192.168.195.122:9100']
        labels:
          ...
          location: 'remote'
          env: 'production'
    scrape_timeout: 30s  # Увеличенный таймаут для удалённого сервера
