...
EXPORT {
    # Разрешаем доступ только клиентам из указанной подсети
    Clients = 192.168.87.0/24;  # либо 192.168.87.*

    # Уникальный идентификатор экспорта
    Export_ID = 1;
    
    # Реальный путь к директории на сервере, которую расшариваем (Основные данные Jira)
    ...
    # Настройки файловой системы (VFS - виртуальная файловая система)
    FSAL {
        Name = VFS;
    }

    # Настройки специфичные для NFSv4
    NFSv4 {
