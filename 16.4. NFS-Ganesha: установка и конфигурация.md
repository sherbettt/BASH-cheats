...
----------------------------------------------------------------------------------
Опционально можно добавить официальный PPA для **NFS-Ganesha** (от команды разработчиков) в **Proxmox (Debian-based)**. 
...
EXPORT
{
    ...
    Protocols = 4;  # Используем NFSv4 (лучше для блокировок)
    Transports = TCP;
    FSAL {
        Name = VFS;
    }
    NFS_Core_Param {
        Enable_NLM = true;  # Включаем поддержку lockd
        NLM_Port = 32803;
    }
}
```
...
> - `No_Root_Squash` — чтобы `root` на клиенте имел доступ.  
