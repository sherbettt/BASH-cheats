...
# Поиск строки 140
cat -n utils/data_generator.py | grep -A 5 -B 5 "^   140"
```
...
sed --in-place 's/pbx.runtel.org/install-test.runtel.ru/g' pyproject.toml

# вставка в середине текста
sed -i '6a\    "--allure-severities=blocker,critical",' pyproject.toml
sed -i -e '5a\' -e '    "--allure-severities=blocker,critical",' pyproject.toml
sed -i '/--alluredir.*test_results/a\    "--allure-severities=blocker,critical",' pyproject.toml
awk 'NR==6{print $0 "\n    \"--allure-severities=blocker,critical\","; next} 1' pyproject.toml > temp && mv temp pyproject.toml
...
# Вывод строк в конкретном диапазоне
sed -n '107,147p' handlers/plugins.yml

# Подстановка в начало строки
sed -i '11,12s/^/#/' requirements.txt
sed -i 's/^runtel-aioesl/#&/; s/^runtel_logger/#&/' requirements.txt
sed -i '/^runtel-aioesl/s/^/#/; /^runtel_logger/s/^/#/' requirements.txt
...
# Вывод строк в конкретном диапазоне
awk 'NR>=107 && NR<=147' handlers/plugins.yml | ccat -n
...
awk -F'==' '{print $1}' requirements.txt  # первый столбец
