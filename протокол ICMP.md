...
```
Команда `tshark` выполняется на той машине, с которой вам необходимо провести мониторинг сетевого трафика. Рассмотрим варианты в зависимости от вашей ситуации:

### Варианты запуска `tshark`

#### 1. Если вы хотите видеть весь трафик, проходящий через роутер:
Выполнять команду нужно именно на самом роутере (машина с тремя интерфейсами). Это даст наиболее полную картину происходящего.

##### Команды для роутера:
```bash
# Захват всего ICMP-трафика
tshark -f 'icmp'

# Только запросы PING (Echo Requests)
tshark -f 'icmp && icmp.type == 8'

# Только ответы на PING (Echo Replies)
tshark -f 'icmp && icmp.type == 0'
```

#### 2. Если интересует конкретный сегмент сети (например, одна из двух клиентов):
Можно запустить `tshark` непосредственно на конкретной клиентской машине (например, 192.168.56.2 или 192.168.56.3). Так вы увидите только локальные события на данной машине.

##### Пример для клиента 192.168.56.2:
```bash
# Захват запросов и ответов ICMP
tshark -f 'icmp'
```
---

Ping-запросы используют протокол ICMP (Internet Control Message Protocol), и они отправляются автоматически операционной системой при выполнении команды `ping`. По сути, выполнение обычного ping-команда включает отправку ICMP-запросов («echo request»), и ожидание ICMP-ответов («echo reply»).

Однако, если вы хотите **фильтровать ICMP-трафик на каком-либо этапе**, то можно управлять этим с помощью инструментов вроде `iptables` (для Linux) или аналогичных решений на Windows/MacOS.

Ниже приведены инструкции по работе с ICMP-фильтрами в среде Linux.

---

### Работа с ICMP-фильтром на Linux с помощью `iptables`

1. **Разрешение ICMP-запросов и ответов:**

   Чтобы разрешить прохождение ICMP-запросов и ответов, можно добавить следующие правила в таблицу `FILTER` (`iptables`):

   ```bash
   # Разрешаем приходящие ICMP-запросы (Echo Request)
   iptables -A INPUT -p icmp --icmp-type echo-request -j ACCEPT

   # Разрешаем уходящие ICMP-ответы (Echo Reply)
   iptables -A OUTPUT -p icmp --icmp-type echo-reply -j ACCEPT
   ```

2. **Отказ от ICMP-трафика (полная блокировка):**

   Полностью закрыть ICMP-трафик можно следующим образом:

   ```bash
   # Блокируем приходящие ICMP-запросы
   iptables -A INPUT -p icmp -j DROP

   # Блокируем исходящие ICMP-ответы
   iptables -A OUTPUT -p icmp -j DROP
   ```

3. **Только разрешённый ICMP-трафик определённых типов:**

   Допустим, вы хотите пропускать только конкретные типы ICMP, например, только ping-запросы (типы 8 и 0):

   ```bash
   # Пропустить только эхо-запросы (type 8)
   iptables -A INPUT -p icmp --icmp-type 8 -j ACCEPT

   # Пропустить только эхо-ответы (type 0)
   iptables -A OUTPUT -p icmp --icmp-type 0 -j ACCEPT
   ```

4. **Ограничение ICMP на основании IP-адресов:**

   Если вы хотите ограничить прохождение ICMP только с определённого IP-адреса или диапазона, это тоже можно реализовать:

   ```bash
   # Принять ICMP от определенного IP (например, 192.168.1.100)
   iptables -A INPUT -p icmp -s 192.168.1.100 -j ACCEPT

   # Заблокировать ICMP от любого другого IP
   iptables -A INPUT -p icmp -j DROP
   ```
...
- Всегда проверяйте, что политика по умолчанию ваших цепей (`INPUT`, `FORWARD`, `OUTPUT`) позволяет приём и отправку пакетов:
  ```bash
  iptables -L -v
  ```

- Для временного отключения всех правил `iptables` можно использовать команду:
  ```bash
  iptables -F
  ```
...
Предположим, у вас есть два устройства:
- Внутренний сервер с IP `192.168.1.100`.
- Внешняя машина с IP `192.168.1.200`.

И вы хотите позволить отправку ping-запросов только от второго устройства на первый:

```bash
# Позволяет получать ICMP-запросы только от 192.168.1.200
iptables -A INPUT -p icmp -s 192.168.1.200 --icmp-type 8 -j ACCEPT

# Отвергнуть остальные ICMP-запросы
iptables -A INPUT -p icmp -j DROP
```
