...
#### 3. Мониторинг с внешней точки зрения:
Вы можете запускать `tshark` на любой машине, имеющей доступ к нужной сети (например, с адреса 192.168.87.136), если эта машина находится в подсети, откуда проходит интересующий вас трафик.

### Итоги:
- Для общего анализа лучше выполнять `tshark` на роутере.
- Если нужен локальный просмотр активности, выполняйте команду на целевой машине (клиентской или внешнем узле).

Таким образом, выбор зависит от цели анализа: глобального обзора всей инфраструктуры или детализированного изучения конкретного узла.
