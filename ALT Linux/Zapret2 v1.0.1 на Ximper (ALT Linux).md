...
<details>
<summary>❗ МЕТОДЫ ФИЛЬТРАЦИИ ❗</summary>

## 📚 **Методы фильтрации трафика в Zapret2 (подробное описание)**

При установке Zapret2 вам будет предложено выбрать метод фильтрации. Каждый метод имеет свои особенности, преимущества и недостатки. Вот подробное описание всех вариантов:

---

### **1️⃣ none (без фильтрации)**
```bash
your choice (default : none) : 1
```

**Что делает:** Обрабатывает ВЕСЬ трафик на указанных портах (80, 443) без каких-либо исключений.

**Как работает:**
- Zapret анализирует и модифицирует ВСЕ HTTPS и HTTP соединения
- Не использует списки сайтов или IP-адресов
- Применяет стратегию обхода к каждому пакету

**✅ Плюсы:**
- Простота настройки (не нужны списки)
- Гарантированно работает для любых сайтов
- Не нужно обновлять списки

**❌ Минусы:**
- Высокая нагрузка на CPU (обрабатывается весь трафик)
- Может замедлять работу обычных сайтов
- Избыточно для большинства задач

**🎯 Когда использовать:** Только если у вас мощный компьютер и вы часто посещаете множество разных заблокированных сайтов, которых нет в стандартных списках.

---

### **2️⃣ ipset (фильтрация по IP-адресам)**
```bash
your choice (default : none) : 2
```

**Что делает:** Обрабатывает трафик только для IP-адресов из заданных списков.

**Как работает:**
- Скачиваются списки IP-адресов заблокированных ресурсов
- Эти IP загружаются в ipset/nftables
- Zapret обрабатывает только соединения к этим IP

**✅ Плюсы:**
- Низкая нагрузка на CPU
- Быстрая обработка (проверка по IP быстрее, чем по доменам)
- Не требует DNS-резолвинга

**❌ Минусы:**
- IP-адреса могут меняться (особенно у CDN типа Cloudflare)
- Нужно часто обновлять списки IP
- Может пропускать сайты, которые сменили IP

**🎯 Когда использовать:** На роутерах или слабых устройствах, где важна производительность, а сайты имеют статичные IP (редкость).

---

### **3️⃣ hostlist (фильтрация по списку доменов) — РЕКОМЕНДУЕТСЯ**
```bash
your choice (default : none) : 3
```

**Что делает:** Обрабатывает трафик только для доменов из заданного списка.

**Как работает:**
1. Скачивается список доменов (например, `youtube.com`, `discord.com`)
2. Программа `mdig` резолвит эти домены в актуальные IP-адреса
3. Полученные IP загружаются в nftables/ipset
4. Zapret обрабатывает только трафик к этим IP

**✅ Плюсы:**
- Оптимальный баланс производительности и эффективности
- Домены резолвятся в актуальные IP при каждом обновлении
- Не требует ручного обновления IP
- Низкая нагрузка на CPU

**❌ Минусы:**
- Нужно периодически обновлять список доменов (автоматически по таймеру)
- При смене IP сайта нужно дождаться следующего обновления (обычно раз в сутки)

**🎯 Когда использовать:** **Это оптимальный выбор для домашнего ПК или ноутбука.** Большинство пользователей выбирают именно этот метод.

---

### **4️⃣ autohostlist (автоматическое обнаружение блокировок)**
```bash
your choice (default : none) : 4
```

**Что делает:** Автоматически определяет, какие сайты блокируются, и применяет обход только к ним.

**Как работает:**
- Изначально не знает, какие сайты заблокированы
- Анализирует неудачные соединения (таймауты, сбросы)
- При обнаружении проблем автоматически добавляет сайт в список обхода
- Со временем "обучается" и собирает свою базу

**✅ Плюсы:**
- Не нужны внешние списки
- Адаптируется под конкретного провайдера
- Автоматически обнаруживает новые блокировки
- Не требует ручного обновления

**❌ Минусы:**
- Первое время может работать неэффективно (пока не накопит статистику)
- Требует больше времени на "обучение"
- Может ошибаться (добавлять работающие сайты или пропускать заблокированные)

**🎯 Когда использовать:** На нестабильных соединениях или когда стандартные списки не подходят. Полезно, если вы часто заходите на редкие сайты, которых нет в публичных списках.

---

## 📊 **Сравнительная таблица методов фильтрации**

| Характеристика | none | ipset | hostlist ⭐ | autohostlist |
|----------------|------|-------|-------------|--------------|
| **Нагрузка на CPU** | Высокая | Низкая | Низкая | Средняя |
| **Требует обновления** | Нет | Да (IP) | Да (домены) | Нет |
| **Точность** | 100% | Средняя | Высокая | Средняя |
| **Сложность настройки** | Просто | Средне | Просто | Сложно |
| **Время на обучение** | Нет | Нет | Нет | Есть (1-2 дня) |
| **Подходит для слабых ПК** | ❌ | ✅ | ✅ | ⚠️ |
| **Рекомендация** | ❌ | ⚠️ | ✅ | ⚠️ |
...
### **Для домашнего ПК или ноутбука:**
```bash
выбираем 3 (hostlist)
```
Это золотая середина — эффективно, производительно, просто.

### **Для роутера или слабого устройства (Raspberry Pi и т.п.):**
```bash
выбираем 2 (ipset) или 3 (hostlist)
```
Оба варианта дадут низкую нагрузку. ipset чуть быстрее, hostlist — точнее.

### **Для экспериментаторов и энтузиастов:**
```bash
выбираем 4 (autohostlist)
```
Интересно понаблюдать, как система сама научится обходить блокировки.

### **Для очень мощного ПК или сервера:**
```bash
выбираем 1 (none)
```
Только если у вас современный многоядерный процессор и вы хотите обрабатывать вообще всё.

---

## 🔧 **Как изменить метод фильтрации после установки**

Если вы выбрали не тот метод, его можно легко изменить:

```bash
# Открываем конфиг
sudo mcedit /opt/zapret2/config

# Находим строку
MODE_FILTER=hostlist

# Меняем на нужное значение:
# MODE_FILTER=none
# MODE_FILTER=ipset
# MODE_FILTER=hostlist
...
Для **99% пользователей** оптимальный выбор — **hostlist (вариант 3)**. Он обеспечивает:
- ✅ Низкую нагрузку на систему
- ✅ Высокую эффективность обхода
- ✅ Простоту использования
- ✅ Автоматическое обновление списков

Остальные методы нужны только в специфических случаях, которые описаны выше.
