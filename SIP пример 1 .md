...
✅ **Логи FreeSWITCH**:  
   ...
   ```
✅ **Доступность порта 5202**:  
   ```bash
   netstat -tuln | grep 5202  # Проверка, слушает ли порт
   tcpdump -i any port 5202   # Проверка, доходят ли пакеты
   ```
✅ **Конфигурация маршрутизации в FreeSWITCH**:  
   ```xml
