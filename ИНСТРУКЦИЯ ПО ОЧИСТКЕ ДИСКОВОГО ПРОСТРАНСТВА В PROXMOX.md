...
1. Разделить хранилища: SSD для систем, HDD для данных
2. Использовать LVM для гибкого управления пространством
3. Настроить квоты для контейнеров
4. Мониторить рост данных приложений

## **ВАЖНЫЕ ПРЕДОСТЕРЕЖЕНИЯ**

1. **Всегда проверяйте** перед удалением
2. **Имейте актуальные бэкапы** критических данных
3. **Останавливайте службы** перед очисткой их данных
4. **Документируйте изменения** для возможности отката
5. **Начинайте с безопасных операций**, переходите к рискованным постепенно

## **РЕЗЮМЕ ПРОЦЕССА**

1. **Диагностика** → `df -h`, `du -ah`, `find`
2. **Анализ** → Определение источников проблемы
3. **Планирование** → От безопасного к рискованному
4. **Выполнение** → Поэтапная очистка с проверкой результатов
5. **Профилактика** → Настройка мониторинга и политик

Этот подход позволяет систематически решать проблемы с дисковым пространством, минимизируя риски для работоспособности системы.
