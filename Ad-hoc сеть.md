...
# Шпаргалка: Настройка сети в ALT Linux (исправление существующего конфига)
...
## Важные заметки для вашего случая:

- **У вас уже есть конфиг** `/etc/systemd/network/eth0.network` - просто добавьте в него DNS
- **Файл без цифрового префикса** тоже работает, но лучше переименовать в `10-eth0.network`
- **После правок конфига** обязательно перезапустите `systemd-networkd`
- **Проверьте что служба запущена** - `systemctl status systemd-networkd`

Основное что нужно было добавить в ваш существующий конфиг - это строки `DNS = 8.8.8.8` и `DNS = 1.1.1.1`
