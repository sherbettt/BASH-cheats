...
```
**Чтобы не прописывать на локальной машине в хостах адреса на ресурсы с абсолютными ссылками (jira, gitlab), можно изменить их в конфиг файле (через # оставил коммент со старой адресацией, которая тоже правильная). Собственно, это можно сделать и для ресурсов с относительными ссылками (grafana и jenkins, например)**
