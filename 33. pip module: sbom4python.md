...
### **Итог**
- Ваш SBOM корректен, но в текстовом формате.  
- Для JSON используйте `--sbom cyclonedx --format json`.  
- Чтобы проверить зависимости, анализируйте `PackageName` в `sbom.spdx`.  

Если нужна помощь с конкретными пакетами или лицензиями — спрашивайте!
