...
# Добавить маршрут в другую сеть через шлюз 192.168.87.1
ip r a 192.168.46.0/24 via 192.168.87.1
ip r a 192.168.45.0/24 via 192.168.87.1
...
EOF
```
**Альттернативный вариант**
```bash
kkorablin@runtel-re ~/projects/git/runtel $ batp /etc/systemd/network/20-wireless.network
[Match]
Name=wlp1s0
...
[Route]
Destination=192.168.46.0/24,192.168.45.0/24
Gateway=192.168.87.1
```
...
### 2.3. Альтернатива: прменяем в NetworkManager
Создаём файлик **`/etc/NetworkManager/dispatcher.d/99-add-routes`**
```bash
#!/bin/bash
INTERFACE=$1
ACTION=$2

if [ "$INTERFACE" = "wlp1s0" ] && [ "$ACTION" = "up" ]; then
    ip route add 192.168.46.0/24 via 192.168.87.1
    ip route add 192.168.45.0/24 via 192.168.87.1
fi
```
