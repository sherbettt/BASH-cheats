...
<details>
<summary>❗Решение с доп. репозиторием❗</summary>
	
-----------------------------
	
У нас **нестандартная установка Proxmox VE (ядро 6.8.12-9-pve) без модулей ядра и репозиториев Proxmox**.  
**Проблема**: `/lib/modules/6.8.12-9-pve` отсутствует, поэтому `nfs` и `nfsd` не загружаются.


### **Решение 1: Установка правильных репозиториев Proxmox VE**
1. **Добавьте официальные репозитории Proxmox**:
   ```bash
...
2. **Установите ядро и модули Proxmox**:
   ```bash
   apt install pve-kernel-6.8 pve-headers-6.8
   update-grub
   reboot
   ```

3. **После перезагрузки проверьте модули**:
   ```bash
...
### **Решение 2: Вручную добавить модули ядра (если репозитории недоступны)**
1. **Скачайте пакет ядра Proxmox вручную**:
   ```bash
   wget http://download.proxmox.com/debian/pve/dists/bookworm/pve-no-subscription/binary-amd64/pve-kernel-6.8.12-9-pve_6.8.12-9_amd64.deb
   dpkg -i pve-kernel-6.8.12-9-pve_6.8.12-9_amd64.deb
   update-initramfs -u
   reboot
   ```

2. **Если пакет недоступен**, попробуйте собрать модули NFS вручную:
   ```bash
   ...
   cp fs/nfs/nfs.ko /lib/modules/$(uname -r)/kernel/fs/nfs/
   depmod -a
   modprobe nfs
   modprobe nfsd
   ```
...
### **Решение 3: Переключиться на стандартное ядро Debian (если Proxmox не нужен)**
1. **Установите ядро Debian**:
   ```bash
...
2. **После перезагрузки**:
   ```bash
   apt install --reinstall nfs-kernel-server
   systemctl start nfs-server
   ```

### **Проверка работоспособности NFS после исправления**
```bash
systemctl start nfs-server
...
#### 🔹 1.1. Настройка NFS-сервера на jira-cluster (192.168.87.140)
